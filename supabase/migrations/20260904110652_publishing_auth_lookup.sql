-- Narrow Auth lookup, callable only by the verified server workflow.
-- Timestamp matches the applied migration.
create function private.publishing_account_created_at(_actor uuid) returns timestamptz
language sql stable security definer set search_path='' as $$
 select created_at from auth.users where id=_actor and _actor is not null
$$;
revoke all on function private.publishing_account_created_at(uuid) from public,anon,authenticated;
grant execute on function private.publishing_account_created_at(uuid) to service_role;
