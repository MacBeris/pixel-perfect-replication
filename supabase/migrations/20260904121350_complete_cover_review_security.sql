-- Complete cover support without changing the existing signed upload flow.
alter table public.plugin_uploads drop constraint plugin_uploads_kind_check;
alter table public.plugin_uploads add constraint plugin_uploads_kind_check check (kind in ('zip','logo','screenshot','banner','cover'));
...
-- Review creation is server-only. Existing owner edits retain column-level grants.
...
returns boolean language sql stable security invoker set search_path='' as $$
 select exists (
  ...
  where p.id=_plugin_id and p.moderation_status='approved'
  and not exists(select 1 from public.developer_profiles d where d.id=p.developer_id and d.owner_id=_user_id)
  and (
   (p.listing_type='direct_sale' and exists (
     select 1 from public.plugin_analytics_events e where e.plugin_id=p.id
     and e.event_type='download' and e.session_hash=md5(_user_id::text||p.id::text)
   ))
   or (p.listing_type='external_listing' and exists (
     select 1 from public.purchases x where x.plugin_id=p.id and x.user_id=_user_id
     and x.status in ('paid','partially_refunded')
   ))
  )
 )
$$;
...
create or replace function public.save_plugin_review(_actor uuid,_plugin_id uuid,_rating integer,_title text,_body text)
returns jsonb language plpgsql security invoker set search_path='' as $$
declare result public.reviews;
begin
 if _actor is null or _rating is null or _rating not between 1 and 5
 or length(coalesce(_title,''))>160 or length(coalesce(_body,''))>5000 then
  raise exception 'Invalid review';
 end if;
 -- Serialize with publication/status changes and concurrent review submissions.
 perform 1 from public.plugins where id=_plugin_id for update;
 if not public.review_download_eligibility(_actor,_plugin_id) then
  raise exception 'Download this plugin through Extendly before leaving a review (external listings require an existing purchase).';
 end if;
 insert into public.reviews(plugin_id,user_id,rating,title,body)
 values(_plugin_id,_actor,_rating,nullif(btrim(_title),''),nullif(btrim(_body),''))
 ...
 if result.id is null then raise exception 'This review is under moderation and cannot be edited.'; end if;
 return jsonb_build_object('id',result.id,'rating',result.rating,'title',result.title,'body',result.body,'updated_at',result.updated_at);
end
$$;
revoke all on function public.save_plugin_review(uuid,uuid,integer,text,text) from public,anon,authenticated;
grant execute on function public.save_plugin_review(uuid,uuid,integer,text,text) to service_role;
