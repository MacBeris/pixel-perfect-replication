-- All publishing mutations are service-only, after server-side getUser(token).
-- Timestamp matches the migration applied by the Supabase connector.
revoke insert, update, delete, truncate, references, trigger on public.plugins, public.plugin_versions,
  public.plugin_assets, public.plugin_categories, public.plugin_tags from anon, authenticated;
alter table public.plugin_versions add column file_verified_at timestamptz;
create unique index plugin_versions_one_current on public.plugin_versions(plugin_id) where is_current;

create table public.plugin_uploads (
  id uuid primary key default gen_random_uuid(),
  plugin_id uuid not null references public.plugins(id) on delete cascade,
  actor_id uuid not null references auth.users(id) on delete cascade,
  version_id uuid references public.plugin_versions(id) on delete cascade,
  kind text not null check(kind in ('zip','logo','screenshot','banner')),
  bucket text not null, staging_path text not null unique, final_path text not null unique,
  original_name text not null, mime text not null, size bigint not null,
  completed_at timestamptz, created_at timestamptz not null default now()
);
alter table public.plugin_uploads enable row level security;
revoke all on public.plugin_uploads from anon, authenticated;
grant all on public.plugin_uploads to service_role;
create index plugin_uploads_pending on public.plugin_uploads(created_at) where completed_at is null;
create index plugin_uploads_plugin on public.plugin_uploads(plugin_id);
create index plugin_uploads_actor on public.plugin_uploads(actor_id);
create index plugin_uploads_version on public.plugin_uploads(version_id);

insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values
 ('plugin-assets','plugin-assets',true,5242880,array['image/jpeg','image/png','image/webp']),
 ('plugin-files','plugin-files',false,52428800,array['application/zip','application/x-zip-compressed','application/octet-stream']);
-- No client object policies: signed uploads are issued only for reserved staging paths.
...
begin
 if not exists(select 1 from auth.users where id=_actor) then raise exception 'Authentication required'; end if;
 if _action='create' then
  ...
  if exists(select 1 from auth.users where id=_actor and created_at>now()-interval '5 minutes') then raise exception 'Your account must be at least 5 minutes old before creating a plugin'; end if;
 ...
 if _action in ('create','save') then
  ...
  if k not in ('zip','logo','screenshot','banner') then raise exception 'Invalid upload kind'; end if;
  if k='zip' then
...
-- Keep the existing moderation UI/RPC and its audit transaction. This trigger
-- publishes the submitted first version in that same transaction.
create function private.publish_first_version() returns trigger language plpgsql security invoker set search_path='' as $$
declare v public.plugin_versions;
begin
 if new.moderation_status='approved' and old.moderation_status='pending_review' and new.listing_type='direct_sale' then
  select * into v from public.plugin_versions where plugin_id=new.id and status='pending_review' and file_verified_at is not null order by created_at limit 1 for update;
  if not found then raise exception 'No verified version is ready for publication'; end if;
  update public.plugin_versions set is_current=false where plugin_id=new.id and is_current;
  update public.plugin_versions set status='published',is_current=true,released_at=now() where id=v.id;
  new.current_version:=v.version_number;
  new.published_at:=coalesce(new.published_at,now());
 end if;
 return new;
end; $$;
revoke all on function private.publish_first_version() from public,anon,authenticated;
create trigger publish_first_version before update of moderation_status on public.plugins for each row execute function private.publish_first_version();
