-- Timestamp matches the applied migration. Reuses tags and includes version changes in admin audit.
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(v)), '[]'::jsonb) into _versions from public.plugin_versions v where plugin_id=_plugin_id;
  ...
  values (_actor_id, 'plugin.' || _status::text, 'plugin', _plugin_id, to_jsonb(_before)||jsonb_build_object('versions',_versions), to_jsonb(_after)||jsonb_build_object('versions',(select coalesce(jsonb_agg(to_jsonb(v)), '[]'::jsonb) from public.plugin_versions v where plugin_id=_plugin_id)), nullif(btrim(_reason), ''));
