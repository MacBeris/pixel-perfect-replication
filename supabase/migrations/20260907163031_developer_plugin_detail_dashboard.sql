...
begin
  ...
  select jsonb_build_object(
    ...
    'activity',coalesce((select jsonb_agg(jsonb_build_object('action',x.action,'created_at',x.created_at) order by x.created_at desc) from (select l.action,l.created_at from public.admin_audit_logs l where l.resource_type='plugin' and l.resource_id=_plugin_id order by l.created_at desc limit 30) x),'[]'::jsonb)
  ) into result;
  return result;
end; $$;

create or replace function public.developer_plugin_detail(_developer_id uuid,_plugin_id uuid)
returns jsonb language sql stable security invoker set search_path='' as $$
  select private.developer_plugin_detail(_developer_id,_plugin_id);
$$;
revoke all on function public.developer_plugin_detail(uuid,uuid) from public,anon;
