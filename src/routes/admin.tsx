...
function AdminPage() {
  ...
  const load = useCallback(
    async (accessToken = session?.access_token) => {
      ...
      <div className="container-page flex min-h-[70vh] items-center justify-center py-12">
        <div className="max-w-xl rounded-xl border border-border bg-card p-7 text-center shadow-sm">
          ...
          <h1 className="mt-4 text-xl font-semibold">
            {loadError ? "Unable to load the admin dashboard" : "Loading admin dashboard…"}
          </h1>
          ...
          {loadError?.startsWith("Missing Supabase environment variable(s):") && (
            <p className="mt-4 text-sm text-muted-foreground">
              Check the server-only runtime variables named in the error above, then deploy the
              updated configuration.
            </p>
          )}
          <Button className="mt-5" onClick={() => void load()} disabled={refreshing}>
    ...
    <div className="space-y-8">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          label="Users"
          value={metrics.users}
          detail={`${metrics.developers} developer profiles`}
        />
        <Metric
          label="Plugins"
          value={dashboard.plugins.length}
          detail={`${metrics.pluginStatuses.pending_review ?? 0} awaiting review`}
        />
        <Metric
          label="Open moderation"
          value={metrics.openClaims + metrics.openReports}
          detail={`${metrics.openClaims} claims · ${metrics.openReports} reports`}
        />
        <Metric
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-6">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">Plugin moderation</h2>
            <Button size="sm" variant="ghost" onClick={() => onNavigate("plugins")}>
              Review queue <ChevronRight className="ml-1 size-4" />
            </Button>
          </div>
          <div className="mt-5 flex flex-wrap gap-2">
            {Object.entries(metrics.pluginStatuses).map(([status, count]) => (
              <Badge key={status} variant={statusVariant(status)}>
                {status.replaceAll("_", " ")}: {String(count)}
              </Badge>
            ))}
            {!Object.keys(metrics.pluginStatuses).length && (
              <span className="text-sm text-muted-foreground">No plugins yet.</span>
            )}
          </div>
        </div>
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="mt-4 space-y-3">
            {dashboard.audit.slice(0, 4).map((entry: any) => (
...
function Plugins({
  ...
  const plugins = dashboard.plugins.filter(
    (plugin: any) => filter === "all" || plugin.moderation_status === filter,
  );
  const update = (plugin: any, status: string) => {
    ...
    void onRun(() =>
      moderatePlugin({ data: { accessToken, pluginId: plugin.id, status: status as any, reason } }),
    );
  };
  const requests = dashboard.changeRequests?.length ? (
    <div className="space-y-3">
      <h3 className="font-semibold">Critical change requests</h3>
      {dashboard.changeRequests.map((request: any) => (
        <div key={request.id} className="rounded-xl border border-border bg-card p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="font-medium">{request.plugin?.name ?? "Plugin change"}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                Requested {date(request.created_at)}
              </p>
              <pre className="mt-3 max-w-2xl overflow-auto rounded-lg bg-muted p-3 text-xs">
                {JSON.stringify(request.changed_fields, null, 2)}
              </pre>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  void onRun(() =>
                    reviewPluginChangeRequest({
                      data: { accessToken, requestId: request.id, approve: true },
                    }),
                  )
                }
              >
              ...
              <Button
                ...
                variant="destructive"
                onClick={() => {
                  const notes = window.prompt("Reason for rejecting these changes:") || "";
                  if (notes)
                    void onRun(() =>
                      reviewPluginChangeRequest({
                        data: { accessToken, requestId: request.id, approve: false, notes },
                      }),
                    );
                }}
              >
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold">Plugin moderation</h2>
          <p className="text-sm text-muted-foreground">
            Approve only listings that meet marketplace quality standards.
          </p>
        </div>
        <select
          value={filter}
      ...
      <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={statusVariant(plugin.moderation_status)}>
                    {plugin.moderation_status.replaceAll("_", " ")}
                  </Badge>
                  {plugin.developer_removed_at && (
                    <Badge variant="destructive">Removed by developer</Badge>
                  )}
                  {plugin.developer_unpublished_at && !plugin.developer_removed_at && (
                    <Badge variant="secondary">Unpublished</Badge>
                  )}
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{plugin.short_description}</p>
                <p className="mt-2 text-xs text-muted-foreground">
                  Developer: {plugin.developer?.name ?? "Unclaimed"} · Submitted{" "}
                  {date(plugin.created_at)}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={() => update(plugin, "approved")}>
                  <Check className="mr-1 size-3.5" />{" "}
                  {plugin.developer_removed_at ? "Restore & approve" : "Approve"}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => update(plugin, "pending_review")}
                >
                  Queue
                </Button>
                <Button size="sm" variant="outline" onClick={() => update(plugin, "suspended")}>
                  Suspend
                </Button>
                <Button size="sm" variant="destructive" onClick={() => update(plugin, "rejected")}>
        ...
        {!plugins.length && <Empty>No plugins match this moderation state.</Empty>}
      </div>
    </div>
  );
}
...
function Workflow({
  ...
  const resolve = (item: any, status: string) => {
    ...
    void onRun(() =>
      updateAdminWorkflow({
        data: { accessToken, kind, itemId: item.id, status: status as any, notes },
      }),
    );
  };
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Review evidence and leave a clear decision trail.
        </p>
      </div>
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id} className="rounded-xl border border-border bg-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold">
                    {item.plugin?.name ??
                      (kind === "reports" ? item.target_type : "Unknown plugin")}
                  </h3>
                  <Badge variant={statusVariant(item.status)}>{item.status}</Badge>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  {kind === "claims"
                    ? item.message || item.evidence || "No evidence supplied."
                    : item.reason}
                </p>
                {kind === "reports" && item.details && (
              ...
              <div className="flex flex-wrap gap-2">
                {kind === "claims" ? (
                  <>
                    <Button size="sm" onClick={() => resolve(item, "approved")}>
                      Approve
                    </Button>
                    <Button
                      size="sm"
                  ...
                  <>
                    <Button size="sm" variant="outline" onClick={() => resolve(item, "reviewing")}>
                      Reviewing
                    </Button>
                    <Button size="sm" onClick={() => resolve(item, "resolved")}>
                      Resolve
                    </Button>
                    <Button
                      size="sm"
        ...
        {!items.length && <Empty>No {kind} need attention.</Empty>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Control marketplace taxonomy without changing application code.
        </p>
      </div>
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        >
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(event) => {
                setKind(event.target.value as any);
                reset();
              }}
          ...
          <Button className="w-full" type="submit">
            {editingId ? "Save changes" : `Add ${kind.slice(0, -1)}`}
          </Button>
          {editingId && (
            <Button className="w-full" type="button" variant="outline" onClick={reset}>
              Cancel editing
            </Button>
          )}
        </form>
        <div className="space-y-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (window.confirm(`Delete ${item.name}? This cannot be undone.`))
                      void onRun(() =>
                        deleteCatalogItem({ data: { accessToken, kind, id: item.id } }),
                      );
                  }}
                >
                  Delete
          ...
          {!items.length && <Empty>No {kind} exist yet.</Empty>}
        </div>
      </div>
    </div>
  );
...
function UsersAndDevelopers({ dashboard }: { dashboard: DashboardData }) {
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-semibold">Users & developers</h2>
        <p className="text-sm text-muted-foreground">
          Role management is intentionally restricted to technical owners outside this panel.
        </p>
      </div>
      <section>
          ...
          <div className="divide-y divide-border">
            {dashboard.users.map((user: any) => (
              <div key={user.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  <p className="font-medium">{user.email ?? "Email unavailable"}</p>
                  <p className="text-xs text-muted-foreground">
                    Joined {date(user.created_at)} · Last sign-in {date(user.last_sign_in_at)}
                  </p>
                </div>
                <Badge variant="secondary">Managed externally</Badge>
              </div>
            ))}
            {!dashboard.users.length && <Empty>No users found.</Empty>}
          </div>
        </div>
      </section>
      <section>
        <h3 className="mb-3 font-semibold">Developer profiles</h3>
        <div className="grid gap-3 sm:grid-cols-2">
          {dashboard.developerProfiles.map((developer: any) => (
            <div key={developer.id} className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="font-medium">{developer.name}</p>
              </div>
              <p className="text-xs text-muted-foreground">
                /{developer.slug} · {developer.is_public ? "Public" : "Private"}
              </p>
            </div>
          ))}
          {!dashboard.developerProfiles.length && <Empty>No developer profiles yet.</Empty>}
        </div>
      </section>
    </div>
  );
...
function Marketplace({ dashboard }: { dashboard: DashboardData }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold">Marketplace operations</h2>
        <p className="text-sm text-muted-foreground">
          Financial records are visible; payment actions remain disabled until Stripe is configured.
        </p>
      </div>
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-5">
        <div className="flex items-center gap-2 font-medium text-amber-700 dark:text-amber-300">
          <CircleDollarSign className="size-4" /> Stripe & Connect not configured
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
          Checkout, transfers, refunds and payouts cannot be created from this environment.
        </p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <Metric label="Purchases" value={dashboard.purchases.length} />
        <Metric label="Transactions" value={dashboard.transactions.length} />
        <Metric label="Payout requests" value={dashboard.payouts.length} />
      </div>
      <section className="rounded-xl border border-border bg-card p-5">
        <h3 className="font-semibold">Recent transactions</h3>
        <div className="mt-4 space-y-3">
          {dashboard.transactions.slice(0, 10).map((transaction: any) => (
...
function ActivityLog({ dashboard }: { dashboard: DashboardData }) {
  const emails = useMemo(
    () => new Map(dashboard.users.map((user: any) => [user.id, user.email])),
    [dashboard.users],
  );
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Append-only record of administrative marketplace actions.
        </p>
      </div>
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <div className="divide-y divide-border">
          {dashboard.audit.map((entry: any) => (
            <div key={entry.id} className="flex flex-wrap items-center justify-between gap-4 p-4">
              <div>
                <p className="font-medium">{entry.action}</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {entry.resource_type} ·{" "}
                  {emails.get(entry.actor_id) ?? entry.actor_id ?? "Deleted administrator"}
          ...
          {!dashboard.audit.length && <Empty>Administrative actions will be recorded here.</Empty>}
        </div>
      </div>
    </div>
  );
