    ...
    >
      <div className="flex items-start gap-3">
        <div className="flex size-11 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border bg-surface font-display text-sm font-semibold text-muted-foreground">
          {plugin.logo_url ? (
            <img src={plugin.logo_url} alt={`${plugin.name} logo`} className="size-full object-cover" loading="lazy" />
          ) : (
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{plugin.platform?.name ?? "Unknown platform"}</p>
        </div>
