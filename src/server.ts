...
import { recordExcludedBotPageRequest } from "./features/analytics/bot-traffic.server";
import { createSitemapResponse } from "./features/seo/sitemap.server";
  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      if (new URL(request.url).pathname === "/sitemap.xml") {
        return await createSitemapResponse(env);
      }
